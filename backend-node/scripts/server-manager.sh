...
restart() { stop; start; }
logs() { p_banner "Backend Logs"; tail -n 100 -f "$LOG_DIR/backend.log"; }
...
case "$cmd" in
  ...
  health) health ;;
  start:dev) start_dev ;;
  *) echo "usage: server-manager.sh [start|start:dev|stop|restart|status|logs|health]" ;;
esac
